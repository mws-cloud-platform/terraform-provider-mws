...
func NodeGroupTestCase(ctx context.Context, sdk *mws.SDK) (acctest.SingleResourceTestCase, error) {
	resourceSDK, err := resourcesdk.NewMk8sNodeGroup(ctx, sdk)
	if err != nil {
	...
	return acctest.SingleResourceTestCase{
		...
		ResourceExists: func(ctx context.Context, id string) error {
			r, err := GetNodeGroup(ctx, resourceSDK, id)
			if err != nil {
		...
		ResourceNotExists: func(ctx context.Context, id string) error {
			_, err := GetNodeGroup(ctx, resourceSDK, id)
			if err != nil {
...
func GetNodeGroup(ctx context.Context, sdk *resourcesdk.Mk8sNodeGroup, id string) (*apimodel.NodeGroupOptionalResponse, error) {
	ref, err := mk8sref.ParseNodeGroupRef(ctx, id)
	if err != nil {
		return nil, fmt.Errorf("parse reference: %w", err)
	}
	return sdk.GetMk8sNodeGroup(ctx, client.GetMk8sNodeGroupRequest{
