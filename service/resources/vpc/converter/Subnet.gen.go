...
func SubnetTFToAPIUpdateRequestModel(ctx context.Context, plan, state *tfmodel.Subnet) (*apimodel.UpdateSubnetRequest, tfdiag.Diagnostics) {
	...
	if !plan.Cidr.Equal(state.Cidr) {
		if !plan.Cidr.IsNull() && !plan.Cidr.IsUnknown() {
			...
			am.Spec.Value.Cidr.SetTo(tmpCidr)
		}
	}
	...
	if !plan.DhcpOptions.Equal(state.DhcpOptions) {
		if !plan.DhcpOptions.IsNull() && !plan.DhcpOptions.IsUnknown() {
			...
			dhcpOptionsState := tfmodel.SubnetDhcpOptions{}
			if !state.DhcpOptions.IsNull() && !state.DhcpOptions.IsUnknown() {
				dhcpOptionsStateDiag := state.DhcpOptions.As(ctx, &dhcpOptionsState, basetypes.ObjectAsOptions{})
				diags = append(diags, dhcpOptionsStateDiag...)
				if diags.HasError() {
					return nil, diags
				}
			}

			dhcpOptionsTmp, dhcpOptionsDiag := SubnetDhcpOptionsTFToAPIUpdateRequestModel(ctx, &dhcpOptionsPlan, &dhcpOptionsState)
			diags = append(diags, dhcpOptionsDiag...)
			if diags.HasError() {
				return nil, diags
			}
