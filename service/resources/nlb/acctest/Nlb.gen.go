...
func NlbTestCase(ctx context.Context, sdk *mws.SDK) (acctest.SingleResourceTestCase, error) {
	resourceSDK, err := resourcesdk.NewNlb(ctx, sdk)
	if err != nil {
	...
	return acctest.SingleResourceTestCase{
		...
		ResourceExists: func(ctx context.Context, id string) error {
			r, err := GetNlb(ctx, resourceSDK, id)
			if err != nil {
		...
		ResourceNotExists: func(ctx context.Context, id string) error {
			_, err := GetNlb(ctx, resourceSDK, id)
			if err != nil {
...
func GetNlb(ctx context.Context, sdk *resourcesdk.Nlb, id string) (*apimodel.NlbOptionalResponse, error) {
	ref, err := nlbref.ParseNlbRef(ctx, id)
	if err != nil {
		return nil, fmt.Errorf("parse reference: %w", err)
	}
	return sdk.GetNlbV2(ctx, client.GetNlbV2Request{
